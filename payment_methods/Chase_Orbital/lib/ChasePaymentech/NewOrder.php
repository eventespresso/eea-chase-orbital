            ...
            if(is_object($response)){
                ...
                if($xml->NewOrderResp){
                    ...
                    if($xml->NewOrderResp->ProcStatus == '0'){
                        ...
                        $this->declined = false;
                        $this->error = true;
                        $this->StatusMsg = 'Gateway rejected request. Gateway responded with message: '.$xml->NewOrderResp;
                        return;

                    }

                } else if ($xml->QuickResponse || $xml->QuickResp){

                        $qr = ($xml->QuickResponse) ? $xml->QuickResponse : $xml->QuickResp;

                        $this->approved = false;
                        $this->declined = false;
                        $this->error = true;
                        $this->ProcStatus = $qr->ProcStatus;
                        $this->StatusMsg = (string) $qr->StatusMsg;
                        return;
